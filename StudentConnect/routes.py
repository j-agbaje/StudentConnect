...
es = Elasticsearch(
    "https://6eb25373254e449fab3f18eac554e1e1.us-central1.gcp.cloud.es.io",
...
def message_request(recipient_id):
    ...
    msg = MailMessage(
        ...
        recipients=[recipient.email],  # The recipient's email address
        ...
        {url_for('chat', room=room, _external=True)}
...
def chat(room):
    if not current_user.is_authenticated:
        flash("You need to log in to access the chat room.", "danger")
        return redirect(url_for('login'))
...
def handle_send_message_event(data):
    app.logger.info(f"{data['sender_id']} has sent a message to the room {data['room']}: {data['message']}")
    data['created_at'] = datetime.now().strftime("%d %b, %H:%M")
    save_message(data['room'], data['message'], data['sender_id'], data['receiver_id'])
    data['username'] = User.query.get(data['sender_id']).username  # Fetch username dynamically
